...
async function getBulkPairData(pairList, ethPrice, chainId) {
  ...
  let [{ number: b1 }, { number: b2 }, { number: bWeek }] = await getBlocksFromTimestamps(chainId, [t1, t2, tWeek])
...
export function Updater() {
  ...
  useEffect(() => {
    async function getData() {
      ...
      let topPairs = await getBulkPairData(formattedPairs, ethPrice, chainId)
...
export function useDataForList(pairList) {
  ...
  }, [ethPrice, state, pairList, stale, fetched, chainId])
...
export function usePairData(pairAddress) {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!pairData && pairAddress) {
        let data = await getBulkPairData([pairAddress], ethPrice, chainId)
  ...
  }, [pairAddress, pairData, update, ethPrice, chainId])
...
/**
 * Get list of all pairs in array
 */
