...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://bsc.array.org/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://bsc.array.org/#/swap?inputCurrency=${
      token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export function getMiningPoolLink(token0Address) {
  return `https://bsc.array.org/#/kwik/ETH/${token0Address}`
}

export function getarrayAppLink(linkVariable) {
  let basearrayUrl = 'https://bsc.array.org/#/kwik'
  if (!linkVariable) {
    return basearrayUrl
  }

  return `${basearrayUrl}/ETH/${linkVariable}`
}
 ...
 */
export async function getBlocksFromTimestamps(chainId, timestamps, skipCount = 500) {
