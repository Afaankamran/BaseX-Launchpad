...
interface ReturnMetrics {
  ...
  arrayReturn: number // netReturn - hodlReturn
  impLoss: number
  fees: number
}
...
export function getMetricsForPositionWindow(positionT0: Position, positionT1: Position): ReturnMetrics {
  ...
  const array_return = difference_fees_usd + imp_loss_usd
 ...
 * @param ethPrice
 */
export async function getLPReturnsOnPair(user: string, pair, ethPrice: number, snapshots, chainId) {
  // initialize values
  ...
  for (const index in snapshots) {
    ...
    arrayReturn = arrayReturn + results.arrayReturn
