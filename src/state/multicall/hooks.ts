...
export type OptionalMethodInputs =
  | BaseX<MethodArg | MethodArg[] | undefined>
  | undefined;
...
export function useMultipleCallMultipleResult(
  contract: Contract | null | undefined,
  methods: { methodName: string; callInputs: OptionalMethodInputs[] }[],
  options?: ListenerOptions,
  gasRequired?: number,
) {
  const calls = useMemo(
    () =>
      methods.length > 0 && contract
        ? methods.map<Call | undefined>(({ methodName, callInputs }) => {
          const fragment = contract?.interface?.getFunction(methodName);
          return methodName && callInputs && isValidMethodArgs(callInputs)
            ? {
              address: contract.address,
