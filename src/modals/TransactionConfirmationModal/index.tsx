...
import { ChainId, Currency } from '@devdot/basex-sdk'
...
import loadingRollingCircle from '../../animation/SwapLoading.json'
...
export const ConfirmationPendingContent: FC<ConfirmationPendingContentProps> = ({
  ...
  return (
    <div>
      ...
      <div className=" w-32 pb-4 m-auto">
        <Lottie animationData={loadingRollingCircle} autoplay loop size={30} />
      </div>
      <div className="flex flex-col items-center justify-center gap-3">
        <div className="text-xl font-bold text-high-emphesis">{i18n._(t`Confirmation...`)}</div>
        <div className="font-bold">{pendingText}</div>
        <div className="font-bold">{pendingText2}</div>
        <div className="text-sm font-bold text-secondary">{i18n._(t`Confirm From Your Wallet`)}</div>
      </div>
    </div>
  )
}
