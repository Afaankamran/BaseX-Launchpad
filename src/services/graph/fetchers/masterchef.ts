...
import { masterChefStaticPairAddressesQuery, staticPoolsQuery } from '../queries/masterchef-static'
...
export const MASTERCHEF_V1 = {
  ...
  [ChainId.MOONRIVER]: 'array1/masterchef',
}

export const masterChefV1 = async (query, chainId = ChainId.MOONRIVER, variables = undefined) =>
  request(`${GRAPH_HOST[chainId]}/subgraphs/name/${MASTERCHEF_V1[chainId]}`, query, variables)

export const MASTERCHEF_STATIC = {
  [ChainId.ROPSTEN]: 'muhammadwajidshahid/masterchefstatic',
  [ChainId.MOONRIVER]: 'array1/static-farms',
}

export const masterChefStatic = async (query, chainId = ChainId.MOONRIVER, variables = undefined) =>
  request(`${GRAPH_HOST[chainId]}/subgraphs/name/${MASTERCHEF_STATIC[chainId]}`, query, variables)
...
export const getMasterChefV1SushiPerBlock = async () => {
  const {
    masterChef: { arrayPerBlock },
  } = await masterChefV1(masterChefV1SushiPerBlockQuery)
  return arrayPerBlock / 1e18
}
...
export const getMasterChefStaticFarms = async (variables = undefined) => {
  const { pools } = await masterChefStatic(staticPoolsQuery, undefined, variables)
  return pools
}

export const getMasterChefStaticPairAddreses = async () => {
  const { pools } = await masterChefStatic(masterChefStaticPairAddressesQuery)
  return pools
}
