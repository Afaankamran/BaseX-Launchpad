...
const usePending = (farm) => {
  ...
  const cloneRewarder = useCloneRewarderContract(farm?.rewarder?.rewarder)

  const complexRewarder = useComplexRewarderContract(farm?.rewarder?.rewarder)
  ...
  const contract = useMemo(
    () => ({
      ...
      [ChainId.ARBITRUM_TESTNET]: complexRewarder,
    }),
