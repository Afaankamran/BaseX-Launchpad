...
import { Token, ZERO } from '@devdot/basex-sdk'
...
const FarmListItemDetails = ({ farm }) => {
  ...
  const liquidityToken =
    isStaticFarm || isNFTStake
      ...
      : new Token(
        ...
        farm.pair.type === PairType.KASHI ? 'KMP' : 'BaseX LP'
      )
  ...
  return (
    <Transition
      ...
      leaveTo="opacity-0"
    >
      <Disclosure.Panel className="flex w-full border-t-0 rounded rounded-t-none bg-dark-900 bg-light-gradient" static>
        {!isStaticFarm && !isNFTStake && !isPairNft ? <InformationDisclosure farm={farm} /> : <></>}
          ...
          {toggleView ? (
            ...
            <Tab.Group>
              <Tab.List className="flex rounded bg-dark-900">
                    ...
                    <Tab
                      className={({ selected }) =>
                        `${selected
                          ...
                          : 'text-secondary'
                        } flex items-center justify-center flex-1 px-2 py-4 text-lg rounded-[6px] cursor-pointer select-none`
                      }
                    >
                ...
                {(isStaticFarm || isNFTStake) && <Tab></Tab>}
              </Tab.List>
              {!isStaticFarm && !isNFTStake && (
                <Tab.Panel>
