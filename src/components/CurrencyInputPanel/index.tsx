...
export default function CurrencyInputPanel({
  ...
  return (
    <div id={id} className={classNames(hideInput ? 'p-4' : '', 'rounded-[15px] bg-gradient-1 p-1')}>
      <div className="flex flex-col justify-between items-center sm:space-y-3  sm:flex-row ">
        <div
          className={classNames(
            'w-full sm:w-2/5 bg-gradient-2 rounded-[15px] pl-5  py-3 sm:mr-5 flex  justify-start  items-center'
          )}
        >
          <button
            type="button"
            ...
            <div className="flex space-x-3 justify-center items-center">
              ...
              <div>
                {pair ? (
                  ...
                  <div className="flex flex-1 flex-col items-start justify-center mx-3.5">
                    ...
                    <div className="flex items-center">
                      <div className="text-lg font-bold token-symbol-container md:text-[20px]">
              ...
              <div>
                {!disableCurrencySelect && currency && (
                  <ChevronDownIcon width={16} height={16} className="  stroke-current" />
                )}
              </div>
            </div>
          </button>
        ...
        <div className="w-[1px] h-12 bg-gradient-3 hidden sm:flex "></div>
        {!hideInput && (
          <div
            className={classNames(
              'flex items-center w-full space-x-2  h-[2.5rem]   focus:bg-dark-700 p-3 py-8 sm:w-3/5'
            ...
            <>
              {showMaxButton && selectedCurrencyBalance && (
                <Button
                  onClick={onMax}
                  size="xs"
                  className="text-xs font-medium bg-transparent border rounded-full hover:bg-primary border-low-emphesis text-secondary whitespace-nowrap w-[50px] order-1 mx-2"
                >
                  {i18n._(t`Max`)}
                </Button>
              )}
