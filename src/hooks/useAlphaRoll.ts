...
import { LPToken } from './useLPTokensState'
import { usearrayRollContract } from './useContract'
import { useActiveWeb3React } from '.'
import ReactGA from 'react-ga'

const useSushiRoll = () => {
  const { library, account } = useActiveWeb3React()
  const sushiRoll = usearrayRollContract()
  const ttl = 60 * 20
