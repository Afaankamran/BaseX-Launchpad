...
import { Currency, CurrencyAmount, Fraction, JSBI, Price } from '@devdot/basex-sdk';
...
export function formatNumberScale(number: any, usd = false) {
  ...
  if (wholeNumberLength >= 13)
    return (usd ? '$' : '') + (num / Math.pow(10, 12)).toFixed(1) + 'T';
  if (wholeNumberLength >= 10)
    return (usd ? '$' : '') + (num / Math.pow(10, 9)).toFixed(1) + 'B';
  if (wholeNumberLength >= 7)
    return (usd ? '$' : '') + (num / Math.pow(10, 6)).toFixed(1) + 'M';
  if (wholeNumberLength >= 4)
    return (usd ? '$' : '') + (num / Math.pow(10, 3)).toFixed(1) + 'K';
...
export function formatDateAgo(date: Date) {
  ...
  if (secondsAgo < 3600)
    return `${Math.floor(secondsAgo / 60)} Minute${secondsAgo / 120 >= 1 ? 's' : ''
      } Ago`;
  if (secondsAgo < 86400)
    return `${Math.floor(secondsAgo / 3600)} Hour${secondsAgo / 7200 >= 1 ? 's' : ''
      } Ago`;
  if (secondsAgo < 2592000)
    return `${Math.floor(secondsAgo / 86400)} Day${secondsAgo / 172800 >= 1 ? 's' : ''
      } Ago`;
  if (secondsAgo < 31536000)
    return `${Math.floor(secondsAgo / 2592000)} Month${secondsAgo / 5184000 >= 1 ? 's' : ''
      } Ago`;
...
export function getFormatedDateDiff(diff) {
  const d = moment.duration(diff);
  return `${d.years() > 0 ? d.years() + ' years, ' : ''}${d.months() > 0 ? d.months() + ' months, ' : ''
    }${d.days() > 0 ? d.days() + ' days, ' : ''}${d.hours() > 0 ? d.hours() + ' hours, ' : ''
    }${d.minutes()} minutes`;
}
export function getFormatedDateDiffSmall(diff) {
  const d = moment.duration(diff);
  return `${d.years() > 0 ? d.years() + ' YY, ' : ''}${d.months() > 0 ? d.months() + ' M, ' : ''
    }${d.days() > 0 ? d.days() + ' D, ' : ''}${d.hours() > 0 ? d.hours() + ' H, ' : ''
    }${d.minutes()} M`;
}
export const toWei = (amount: string) => {
  return parseUnits(amount, '18');
};
export const fromWei = (amount: BigNumberish, decimals?: number) => {
